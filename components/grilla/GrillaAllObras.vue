                ...
                <table class="table-auto">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in paginatedItems" :key="index" @click="selectRow(item)">
                            ...
                            <td>
                                <NuxtLink :to="`/dashboard/${item.id}`" @click="console.log(item.id)">ver Obra
                                </NuxtLink>
                            </td>
                            <td v-for="(field, index) in fieldSettings" :key="field.id">
                                <div v-if="cellRefs" class="flex flex-grow " :ref="el => cellRefs[index] = el">
                                    ...
                                    <div v-else-if="field.type === 'price'" :style="{ width: `50px` }"
                                        class="flex flex-grow items-center justify-start px-2 py-1 rounded-md bg-white"
                                        :class="{ 'bg-gray-300': !field.isEditable && isEditing, 'bg-white': field.isEditable }">
...
export default {
    ...
    methods: {
        ...
        async fetchNombre(id: number, fieldName: string) {
            ...
            switch (fieldName) {
                case 'clienteId':
                    url = `http://localhost:3333/clientes/${id}`;
                    console.log('url: ', url);
                    break;
        ...
        consulta: {
            ...
            handler(newValue) {
                for (let item of newValue) {
                    for (let field in item) {
                        if (field === 'clienteId') { // Añade más campos si es necesario
    ...
    setup(props, { emit }) {
        ...
        let consulta = ref<Obra[]>([]);
        let open = ref(true);
        let selected = ref<Obra | null>(null); // Nueva variable para almacenar el objeto ArtTarea seleccionado
        ...
        async function refreshData() {
            ...
            console.log('rutaGet desde all obras: ', props.rutaGet);
            const response = await fetch(props.rutaGet);
            if (response.ok) {
                const contentType = response.headers.get("content-type");
                const camposResponse = await fetch(`http://localhost:3333/user_field_settings/table/obras`);  // campos editables
        ...
        onMounted(async () => {
            ...
            // Cargar cliente
            const responsePre = await fetch('http://localhost:3333/clientes');
            if (responsePre.ok) {
        ...
        const selectRow = (row: Obra) => {
            if (showModalClientes.value == false) {
                selected.value = row;

                console.log('Row selected: ', row.id);
                console.log('selectedArticulo: ', selected.value);
            } else {
                console.log('Row selected: rota ' + 'showModalRubros: ' + showModalClientes.value + 'showModalMarcas: ');
            }
        };

        // eliminar una fila visualmente
        const deleteRowVisual = (rowToDelete: Obra) => {
        ...
        const updateRow = (newId: any, modalField: string) => {
            if (selected.value !== null && newId !== undefined) {
                switch (modalField) {
                    case 'clienteId':
                        selected.value.clienteId = newId.id;
                        console.log('selectedRowModal clienteId: ', selected.value.clienteId);
                        break;
                    default:
