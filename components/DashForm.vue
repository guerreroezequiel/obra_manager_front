...
export default {
    ...
    setup(props, { emit }) {
        ...
        const nuevaCopia = async (selectedId: number) => {
            ...
            console.log(`http://localhost:3333/tareas/copy/${selectedId}`)
            const response = await fetch(`http://localhost:3333/tareas/copy/${selectedId}`, {
                method: 'POST',
                headers: {
